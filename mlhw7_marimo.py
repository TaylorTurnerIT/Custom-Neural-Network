...
@app.cell
def _(MinMaxScaler, PolynomialFeatures, X_test, X_train, X_validation):
...
def _(mo):
    ...
    **Part 4 Interpretation:**
    ...
    Polynomial features gave the model access to interaction terms and squared features that the linear boundary couldn't represent, and that shows up in the numbers — both precision and recall improve across both classes compared to the 10-feature baseline. The hadron class benefits the most since the extra features help carve out the parts of the space where hadrons cluster but the linear boundary was missing them. The val loss curves track the training loss closely in both cases, so neither model is meaningfully overfitting — this is expected since logistic regression has no hidden capacity to blow up even with 65 features at this sample size. The improvement confirms that the data isn't linearly separable and that the baseline was leaving real signal on the table.
    """)
    return
